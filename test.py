...
def test(KSTEPS=20, scale=0.05):
    ...
    for batch in loader_test:
        ...
        for k in range(KSTEPS):  # the value of KSTEPS is 20
            ...
            # # draw graph
            # if step == 13 or step == 14 or step == 99:
            #     edge_colors = []
            ...
            #         plt.plot(obs_traj[0, i, 0, :].cpu().numpy(), obs_traj[0, i, 1, :].cpu().numpy(), 'm.-.',
            #                  color='blue',
            #                  label='training accuracy')
            #         plt.plot(pred_traj_gt[0, i, 0, :].cpu().numpy(), pred_traj_gt[0, i, 1, :].cpu().numpy(), 'm.-.',
            ...
            if step == tbd_cnt:  # 241
                for i in range(obs_traj.shape[1]):  # traversing the id of each pedestrian
                    ...
                    plt.plot(pred_traj_gt[0, i, 0, :].cpu().numpy(), pred_traj_gt[0, i, 1, :].cpu().numpy(), 'm.-.',
                             color='red', label='training accuracy')
                    plt.plot(V_pred_rel_to_abs[:, i, 0], V_pred_rel_to_abs[:, i, 1], 'm.-.', color='green')
                #plt.title("Batch:" + str(step))
                #plt.show()
            #
            ...
            # if step == 241:
            #     for i in range(obs_traj.shape[1]):
            #         plt.plot(V_pred_rel_to_abs[:, i, 0], V_pred_rel_to_abs[:, i, 1], color='red')
            #         plt.plot(V_y_rel_to_abs[:, i, 0], V_y_rel_to_abs[:, i, 1], color='green')
            #         plt.show()
            #     pass
            for n in range(num_of_objs):  # traversing the id of each pedestrian
                pred = []
...
if __name__ == '__main__':
    paths = ['./checkpoint/*hgcnsi*']  # paths=['./checkpoint/*hgcnsi*']
    # KSTEPS = 20
    KSTEPS = 20
    print("*" * 50)
    ...
    #     ade_ls = []
    #     fde_ls = []
    #     path = paths[feta]  # path='./checkpoint/*hgcnsi*'
    #     exps = glob.glob(path)
    #     print('Model being tested are:',
    #           exps)  # exps=['./checkpoint\\hgcnsi-eth', './checkpoint\\hgcnsi-hotel', './checkpoint\\hgcnsi-univ', './checkpoint\\hgcnsi-zara1', './checkpoint\\hgcnsi-zara2']
    #     cnt_cnt = 0
    ...
    #     for exp_path in exps:
    #         cnt_cnt += 1
    #         if cnt_cnt == 1 or cnt_cnt==2 or cnt_cnt==3 or cnt_cnt==4:
    #             continue
