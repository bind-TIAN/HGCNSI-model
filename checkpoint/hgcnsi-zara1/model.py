...
class SceneAttentionShare(nn.Module):
    ...
    def forward(self, x, A, vgg):
        ...
        # sequential_scene_attention_result = torch.zeros(1, T, sequential_scene_attention.shape[0],
        #                                                 sequential_scene_attention.shape[1]).cuda()
        # for i in range(T):
        #     sequential_scene_attention_i = sequential_scene_attention[:, i]
        #     sequential_scene_attention_i = sequential_scene_attention_i.unsqueeze(1)
        #     sequential_scene_attention_i = sequential_scene_attention_i.permute(1, 0, 2)
        #     sequential_scene_attention_i = sequential_scene_attention_i.unsqueeze(0)
        #     sequential_scene_attention_result[:, i] = sequential_scene_attention_i

        # sequential_scene_attention = sequential_scene_attention.squeeze(-1)
        # sequential_scene_attention = sequential_scene_attention.squeeze(1)
        #
        # # sequential_scene_attention.shape:torch.size([1,npeds,8])
        # sequential_scene_attention = sequential_scene_attention.unsqueeze(0)
        #
        # # sequential_scene_attention.shape:torch.size([1,1,npeds,8])
...
class SequentialSceneAttention(nn.Module):
    ...
    def forward(self, vgg, end_pos):  # the end_pos means the last observed time step t.
        ...
        # 2023.4.11, do not change end_pos
        # end_pos = end_pos[0, :, :]
        # curr_rel_embedding = self.spatial_embedding(end_pos)# curr_rel_embedding.shape:[npeds,observed_length,10]
        curr_rel_embedding = self.spatial_embedding(end_pos)  # curr_rel_embedding.shape:[npeds,10]

        # curr_rel_embedding.view(-1, 1, self.embedding_dim).shape:[npeds, 1, 10]
        # curr_rel_embedding.shape:[npeds,196,10]
        curr_rel_embedding = curr_rel_embedding.view(-1, 1, self.embedding_dim).repeat(1, self.L, 1)

        # vgg.shape:torch.size([1,14,14,512])
        # vgg.shape:torch.size([npeds,14,14,512])
        # vgg.shape:torch.size([npeds*196,512])
        # 2023.4.11
        vgg = vgg.repeat(end_pos.shape[1] * npeds, 1, 1, 1)
        # vgg = vgg.repeat(npeds, 1, 1, 1)
        vgg = vgg.view(-1, self.D)

        # features_proj.shape:torch.size([npeds*196,16])
        # features_proj.shape:torch.size([npeds,196,16])
        features_proj = self.pre_att_proj(vgg)
        features_proj = features_proj.view(-1, self.L, self.D_down)

        # features_proj.shape:torch.size([npeds,196,16])
        # curr_rel_embedding.shape:torch.size([npeds,196,10])
        # mlp_h_input.shape:torch.size([npeds,196,26])
        mlp_h_input = torch.cat([features_proj, curr_rel_embedding], dim=2)

        # mlp_h_input.view(-1, self.embedding_dim + self.D_down).shape:torch.size([npeds*196,26])
        # mlp_h_input.view(-1, self.embedding_dim + self.D_down)->make_mlp->attn_h.shape:torch.size([npeds*196,8])
        # attn_h.shape:torch.size([npeds,196,8])
        attn_h = self.mlp_pre_attn(mlp_h_input.view(-1, self.embedding_dim + self.D_down))
        attn_h = attn_h.view(end_pos.shape[1] * npeds, self.L, self.bottleneck_dim)

        # attn_h.view(npeds, -1).shape:torch.size([npeds,196*8])
        # self.attn(attn_h.view(npeds, -1)).shape:torch.size([npeds,196])
        # attn_w.shape:torch.size([npeds,196])
        # attn_w.shape:torch.size([npeds,196,1])
        attn_w = Func.softmax(self.attn(attn_h.view(end_pos.shape[1] * npeds, -1)), dim=1)
        attn_w = attn_w.view(end_pos.shape[1] * npeds, self.L, 1)

        # attn_h.shape:torch.size([npeds,196,8])
        # attn_w.shape:torch.size([npeds,196,1])
        # attn_h * attn_w.shape:torch.size([npeds,196,8])
        # sequential_scene_attention.shape:torch.size([npeds,8])
